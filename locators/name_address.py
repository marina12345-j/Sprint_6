...
class NameAddress(BasePageLocators):
    ...
    METRO_STATION = (By.XPATH, "//div[@class='select-search__value']" ),# Поле Станция метро
    METRO_STATION_LIST = (By.CLASS_NAME, 'select-search__select')# Список станций метро
    SELECTED_STATION = (By.XPATH, "//input[@class='select-search__input']/button[@value='Бульвар Рокоссовского']"), # Выбранная станция метро
    FIELD_TELEPHONE = (By.XPATH, "//div[@class='Order_Form__17u6u']/div[@class='Input_InputContainer__3NykH']/input[@placeholder='* Телефон: на него позвонит курьер']"), # поле Телефон
    BUTTON_FURTHER = (By.XPATH, "//button[text() = 'Далее']"),# кнопка Далее
...
# Поле про аренду
    ...
    FIELD_COMMIT = (By.XPATH, "//input[@class= 'Input_Input__1iN_Z Input_Responsible__1jDKN']/input[@placeholder='Комментарий для курьера']") # поле Комментарий для курьера
...
# Хотите оформить заказ?
    BUTTON_OK = (By.XPATH, "//button[text()='Нет']") # кнопка Да
